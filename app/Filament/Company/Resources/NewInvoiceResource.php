                ...
                // Grid::make('GRID')->columnSpan(3)->schema([
                    ...
                    Section::make('Totali')->columns(4)
                        ...
                        ->schema([
                            ...
                            Forms\Components\TextInput::make('total_notes_payment')->label('Note di credito e pagamenti')
                                ->extraInputAttributes(['style' => 'text-align: right;'])
                                ->formatStateUsing(fn (?Invoice $record): ?string => $record
                                    ? number_format($record->total_notes + $record->total_payment, 2, ',', '.')
                                    : null)
                                ->suffix('€')
                                ->columnSpan(6)
                                ->disabled()
