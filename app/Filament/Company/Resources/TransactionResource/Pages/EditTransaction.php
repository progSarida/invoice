...
class EditTransaction extends EditRecord
{
        ...
        return [
            ...
            Actions\Action::make('previous_d_transaction')
                ...
                ->action(function () use ($previousDTransaction) {
                    $this->redirect(TransactionResource::getUrl('edit', ['record' => $previousDTransaction->id]));
                }),
            Actions\Action::make('next_d_transaction')
                ...
                ->action(function () use ($nextDTransaction) {
                    $this->redirect(TransactionResource::getUrl('edit', ['record' => $nextDTransaction->id]));
                }),
            Actions\Action::make('previous_i_transaction')
                ...
                ->action(function () use ($previousITransaction) {
                    $this->redirect(TransactionResource::getUrl('edit', ['record' => $previousITransaction->id]));
                }),
            Actions\Action::make('next_i_transaction')
                ...
                ->action(function () use ($nextITransaction) {
                    $this->redirect(TransactionResource::getUrl('edit', ['record' => $nextITransaction->id]));
                }),
    ...
    protected function getDeleteFormAction()
    {
        return Actions\DeleteAction::make('delete')
                ...
                ->modalHeading('Conferma eliminazione voce prima nota')
                ->modalDescription('Sei sicuro di voler eliminare questa voce? Questa azione non può essere annullata.')
    ...
    protected function getCancelFormAction(): Actions\Action
    {
        return Actions\Action::make('cancel')
            ...
            ->color('gray')
            ->url(function () {
                if ($this->previousUrl && str($this->previousUrl)->contains('/transactions?')) {
